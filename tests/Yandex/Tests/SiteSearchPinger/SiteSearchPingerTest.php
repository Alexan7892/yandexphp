...
class SiteSearchPingerTest extends TestCase
{
    ...
    function testPingResponse()
    {
        ...
        $xml                         = file_get_contents(__DIR__ . 'SiteSearchPingerTest.php/' . $this->fixturesFolder . '/ping-response.xml');
    ...
    function testPingResponseError()
    {
        ...
        $xml                  = file_get_contents(
            __DIR__ . 'SiteSearchPingerTest.php/' . $this->fixturesFolder . '/ping-response-error.xml'
        );
        $response             = new Response(200, [], \GuzzleHttp\Psr7\stream_for($xml));
    ...
    function testPingResponseEmptyParam()
    {
        ...
        $searchId             = '111111';
        $xml                  = file_get_contents(
            __DIR__ . 'SiteSearchPingerTest.php/' . $this->fixturesFolder . '/ping-response-empty-param.xml'
        );
        $response             = new Response(200, [], \GuzzleHttp\Psr7\stream_for($xml));
        $guzzleHttpClientMock = $this->getMockBuilder(GuzzleHttpClient::class)
            ->setMethods(['request'])
    ...
    function testPingResponseIncorrectSearchId()
    {
        ...
        $xml                  = file_get_contents(
            __DIR__ . 'SiteSearchPingerTest.php/' . $this->fixturesFolder . '/ping-response-error-incorrect-search-id.xml'
        );
        $request              = new Request('POST', '');
        $response             = new Response(200, [], \GuzzleHttp\Psr7\stream_for($xml));
        $exception            = new \GuzzleHttp\Exception\ClientException('error', $request, $response);
