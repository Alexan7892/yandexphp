...
class DatabaseSnapshotTest extends TestCase
{
    ...
    {
        $json     = file_get_contents(__DIR__ . 'DatabaseSnapshotTest.php/' . $this->fixturesFolder . '/database-snapshot.json');
    ...
    {
        $json     = file_get_contents(__DIR__ . 'DatabaseSnapshotTest.php/' . $this->fixturesFolder . '/get-database-snapshot-filtered.json');
        $jsonObj  = json_decode($json);
        $response = new Response(200, [], \GuzzleHttp\Psr7\stream_for($json));
        $dataSyncClientMock = $this->getMockBuilder(DataSyncClient::class)
            ->setMethods(['sendRequest'])
